...
这是现有 ArtHub 网页查看器的静态部署包。网页端当前主要测试 NIKKE，同时恢复雀魂和棕色尘埃2动态查看器；不包含 Mod 页面、Mod 资源或静态图库。
...
- 棕色尘埃2入口保留，后续继续完善。
- BD2 Mod、NIKKE Mod 和静态图库不进入网页入口、网页索引或 COS 上传清单。
...
4. 先测试 NIKKE 的缩略图和按需加载，再测试雀魂或 BD2。
